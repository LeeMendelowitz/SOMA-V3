                                             ...
                                             const ContigMapData * pContigMap, bool contigIsForward)
{
    ...
    for(std::vector<FragData>::const_iterator iter = contigFrags.begin();
    ...
    for(std::vector<Index_t>::const_iterator ti = tb+1; ti != te; ti++)
    {
        ...
        #if BUILDMATCH_DEBUG > 0
        {
            ...
            int cSize = 0;
            for(std::vector<FragData>::const_iterator iter = chunk.getContigFragB(); 
            ...
            int oSize = 0;
            for(std::vector<FragData>::const_iterator iter = chunk.getOpticalFragB(); 
